...
export default function Withdrawal() {
  ...
  const [ethToInrRate, setEthToInrRate] = useState<number>(18.4);
  const [templeFundAddress, setTempleFundAddress] = useState<string>("");
  const [templeBlockchainAddress, setTempleBlockchainAddress] = useState<string | null>(null);
  const [templeAdminData, setTempleAdminData] = useState<any>(null);

  // Update contract address when chainId changes
  useEffect(() => {
    if (chainId) {
      const address = getTempleFundAddress(chainId);
      setTempleFundAddress(address);
      console.log(`Using TempleFund address: ${address} on chain ${chainId}`);
    }
  }, [chainId]);

  // Fetch current temple admin info
  useEffect(() => {
    const fetchTempleAdminInfo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setTempleAdminData(admin);
          
          // Get the blockchain address for this temple
          if (admin && admin.templeName) {
            const blockchainAddr = getTempleBlockchainAddress(admin.templeName);
            setTempleBlockchainAddress(blockchainAddr);
            console.log(`Temple: ${admin.templeName}, Blockchain Address: ${blockchainAddr}`);
          }
        } else {
          console.warn("Failed to fetch temple admin info");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (selectedCrypto && selectedCrypto !== "matic" && selectedCrypto !== "ethereum") {
      toast.info(
  ...
  const fetchTempleBalance = async (templeAddr: string) => {
    if (!provider || !ethers.isAddress(templeAddr)) {
      console.warn("Invalid temple address or wallet not connected.", { templeAddr, hasProvider: !!provider });
      return;
    }
    try {
      ...
      if (!signer) {
        console.warn("Could not get signer from provider");
        return;
      }

      if (!templeFundAddress) {
        console.warn("templeFundAddress is not configured");
        return;
      }
      ...
      // Update cryptoBalances for both Matic and Ethereum (they share the same balance)
      setCryptoBalances((prev) =>
        prev.map((crypto) =>
          (crypto.id === "matic" || crypto.id === "ethereum")
            ? {
                ...crypto,
      ...
      console.warn("Failed to fetch temple balance:", error?.message || error);
      // Don't show error toast, silently fail with fallback (0 balance)
      setMaticBalance("0");
    }
  };

  useEffect(() => {
    if (!provider || !templeBlockchainAddress) return;

    fetchTempleBalance(templeBlockchainAddress); // Fetch using temple's blockchain address
  ...
  }, [provider, templeBlockchainAddress]);
  ...
  const withdrawEth = async () => {
    ...
    if (selectedCrypto !== "matic" && selectedCrypto !== "ethereum") {
      toast.info(
        `${selectedCrypto.toUpperCase()} withdrawals are not available yet.`
      );
      return;
          ...
          >
            ...
            <div className="space-y-6">
              ...
              {purpose === "other" && (
                <div>
                  ...
                  <input
                    type="text"
                    value={customReason}
                    disabled={selectedCrypto !== "matic" && selectedCrypto !== "ethereum"}
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={withdrawEth}
                disabled={(selectedCrypto !== "matic" && selectedCrypto !== "ethereum") || txLoading}
                className={`w-full py-4 rounded-xl font-medium shadow-lg flex items-center justify-center space-x-2 ${
                  selectedCrypto !== "matic" && selectedCrypto !== "ethereum"
