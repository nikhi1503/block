...
export default function Donations() {
  ...
  const totalPages = Math.ceil((donations?.length || 0) / itemsPerPage);
  ...
  useEffect(() => {
    const fetchTempleDonations = async () => {
      try {
        ...
        const res = await fetch("http://localhost:5500/api/v1/transactions/temple-donations", {
          headers: {
    ...
    <AuthWrapper role="templeAdmin">
      <div className="p-8 space-y-6 h-full overflow-y-auto">
        ...
        >
          ...
          {/* Empty State */}
          {!isLoading && !error && currentDonations.length === 0 && (
          ...
          {/* Table */}
          {!isLoading && currentDonations.length > 0 && (
