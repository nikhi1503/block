...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchRecent = async () => {
      try {
        ...
        const res = await fetch("http://localhost:5500/api/v1/transactions/recent-temple-donations", {
          headers: {
  ...
  useEffect(() => {
    const fetchTrends = async () => {
      try {
        const token = sessionStorage.getItem("accessToken");
        const res = await fetch("http://localhost:5500/api/v1/transactions/temple-monthly-donations", {
          headers: {
            "Authorization": `Bearer ${token}`,
          },
        });
  ...
  // Note: Removed superAdminDashboard endpoint call as temple admins don't have access
  // useEffect(() => {
  //   const fetchUserCount = async () => {
  //     try {
  //       const res = await fetch("http://localhost:5500/api/v1/superAdminDashboard/count-users");
  //       const result = await res.json();
  ...
  useEffect(() => {
    const fetchTotalDonations = async () => {
      try {
        const token = sessionStorage.getItem("accessToken");
        const res = await fetch("http://localhost:5500/api/v1/transactions/temple-total-donations", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
