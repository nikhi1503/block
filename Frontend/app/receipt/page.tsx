...
export default function GlobalDonationSuccessPage() {
      ...
      try {
        ...
        try {
          ...
          const response = await axios.get(
            `http://localhost:5500/api/v1/transactions/receipt?txHash=${txHash}`,
            {
              headers,
            }
          );
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            console.log("Receiver data:", donationData.receiver);

            const normalizedDonationDetails = {
              ...donationData,
              templeName: donationData.receiver?.templeName || "Temple",
              templeLocation: donationData.receiver?.templeLocation || "Location",
              // Use blockchain wallet addresses if available, fallback to user wallet addresses
              templeWalletAddress: donationData.templeWalletAddress || donationData.receiver?.walletAddress,
              senderWalletAddress: donationData.senderWalletAddress || donationData.sender?.walletAddress,
              senderName: (
                donationData.sender?.name || "Anonymous"
              ).toUpperCase(),
              // Ensure createdAt is present for date display
              createdAt: donationData.createdAt,
            };

            console.log("Normalized donation details:", normalizedDonationDetails);
          ...
          console.warn("Database fetch failed, trying blockchain lookup:", dbError);
          // Fall through to blockchain fetch
        }

        // Fallback: Fetch transaction details directly from blockchain
        console.log("Fetching transaction from blockchain...");
        
        // Try multiple RPC endpoints for better resilience
        const rpcEndpoints = [
          "http://localhost:8545", // Local Hardhat
          "https://eth-sepolia.g.alchemy.com/v2/demo", // Fallback public RPC
        ];
        ...
        for (const rpcUrl of rpcEndpoints) {
          try {
            ...
            blockchainReceipt = await Promise.race([
            ...
            if (blockchainReceipt) {
              console.log("Transaction found on blockchain:", blockchainReceipt);
        ...
        if (blockchainReceipt) {
          // Get full transaction details to extract donation amount
          ...
          try {
            ...
            if (tx && tx.data) {
              // Try to decode the donation amount from transaction input data
              // The function call is donateEthToTemple(address, {value: amount})
              // The value is passed separately, not in the call data
              // Check if there's a value transfer
              if (tx.value && tx.value > 0n) {
                donationAmount = formatUnits(tx.value, 18);
              } else {
                // Fallback: use receipt value if available
                donationAmount = formatUnits(blockchainReceipt.value || "0", 18);
              }
            }
          } catch (e) {
            console.warn("Could not get full transaction details:", e);
            donationAmount = formatUnits(blockchainReceipt.value || "0", 18);
          }
          
          // Create a minimal donation details object from blockchain data
          const blockchainDonationDetails = {
            txHash: txHash,
            amount: donationAmount,
            templeName: "Temple Donation (Blockchain Record)",
            templeLocation: "Registered on Hardhat Local Network",
            senderWalletAddress: blockchainReceipt.from,
            templeWalletAddress: blockchainReceipt.to,
            senderName: "DONOR",
            status: blockchainReceipt.status === 1 ? "confirmed" : "failed",
            blockNumber: blockchainReceipt.blockNumber,
            gasUsed: blockchainReceipt.gasUsed.toString(),
            timestamp: new Date().toISOString(),
            source: "blockchain", // Flag to indicate blockchain origin
          };
          
          console.log("Blockchain donation details:", blockchainDonationDetails);
          setDonationDetails(blockchainDonationDetails);
          ...
          toast.info("✅ Transaction confirmed on blockchain!");
        } else {
          // All RPC endpoints failed or transaction not found
          console.error("Could not reach any RPC endpoint:", lastError);
          
          // Create a placeholder receipt showing the transaction hash
          // This proves the transaction was submitted and confirmed
          const placeholderDetails = {
            txHash: txHash,
            templeName: "Temple Donation",
            senderName: "DONOR",
            status: "pending_verification",
            source: "pending",
            message: "Your transaction has been submitted to the blockchain. Verification is pending.",
          };
          
          setDonationDetails(placeholderDetails);
          toast.warning("Transaction submitted to blockchain. Verification details coming soon...");
  ...
  const readableTransactionFee =
    donationDetails?.transactionFee !== undefined
      ? typeof donationDetails.transactionFee === 'string'
        ? donationDetails.transactionFee
        : Number(donationDetails.transactionFee).toFixed(6)
      : null;
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share) {
        ...
        const shareText = `I just donated to ${donationDetails.templeName} on DevTemple!\n${window.location.href}`;
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-6">
        <Link
          href={`/temples/${(donationDetails.templeName || "temple")
            .toLowerCase()
            .replace(/\s+/g, "-")}-${(donationDetails.templeLocation || "location")
            .toLowerCase()
            .replace(/\s+/g, "-")}`}
