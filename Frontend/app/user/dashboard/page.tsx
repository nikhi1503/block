...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchUserTotalDonations = async () => {
      ...
      try {
        const res = await fetch("http://localhost:5500/api/v1/transactions/total-donation-done", {
          headers: {
    ...
    const fetchMaticToINR = async () => {
      ...
      while (retries < maxRetries) {
        try {
          ...
          if (!res.ok) {
            console.warn(`API error: ${res.status}, retrying...`);
            retries++;
          } else {
            const data = await res.json();
            ...
            if (price && typeof price === 'number' && price > 0) {
              ...
              console.warn(`Invalid price data: ${price}, retrying...`);
              retries++;
            }
          }
        } catch (error) {
          retries++;
          console.warn(`Error fetching MATIC to INR (attempt ${retries}/${maxRetries}):`, error);
        }
        
        if (retries >= maxRetries) {
          // All retries exhausted, use fallback
          console.warn("Using fallback MATIC price");
          setMaticToInr(90);
          return;
        }
  ...
  useEffect(() => {
    const fetchVerifiedCount = async () => {
      try {
        const response = await fetch("http://localhost:5500/api/v1/superAdminDashboard/count-temple-admins");
  ...
  useEffect(() => {
    const fetchMyDonations = async () => {
      ...
      try {
        const response = await fetch("http://localhost:5500/api/v1/transactions/my-donations", {
          headers: {
        ...
        if (response.ok) {
          ...
          console.warn("Could not fetch donations:", result.message);
        }
      } catch (error) {
  ...
  const refreshDonations = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.warning("No recent donations found. Check the receipt page for blockchain records.");
      }
    } catch (error) {
      console.error("Failed to fetch donations:", error);
      toast.error("Could not refresh donations");
    } finally {
      setRefreshing(false);
    }
  };
  ...
  useEffect(() => {
    const fetchMonthlyDonations = async () => {
      try {
        const token = sessionStorage.getItem("accessToken");
        const res = await fetch("http://localhost:5500/api/v1/transactions/temple-donated-amount", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center space-x-2 px-3 py-1 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-200 transition-all disabled:opacity-50"
              title="Refresh donations from database"
            >
          ...
          {recentDonations && recentDonations.length > 0 ? (
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-semibold text-gray-800 text-sm">
                            {donation.receiver?.templeName || "Temple"}
                          </p>
                          <p className="text-gray-600 text-xs">{donation.purpose || "Temple Donation"}</p>
                          {donation.createdAt && (
            ...
            <div className="text-center text-gray-600 p-6 space-y-4">
              <HandHeart className="mx-auto text-orange-500" size={32} />
              <p className="text-sm font-medium">No donations have been recorded in the database yet.</p>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-xs text-blue-800">
                  💡 If you just made a donation, your transaction is recorded on the blockchain! Check your receipt page to view details, or click the Refresh button above to sync with the database.
                </p>
              </div>
              <button
              ...
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-full max-w-xl shadow-xl relative">
                  ...
                  <div className="space-y-3 text-sm text-gray-700">
                    ...
                    <p><strong>Purpose:</strong> {selectedDonation.purpose || "N/A"}</p>
                    ...
                    <p><strong>Sender Name:</strong> {selectedDonation.sender?.name || sessionStorage.getItem("user_data") ? JSON.parse(sessionStorage.getItem("user_data") || "{}").name : "N/A"}</p>
                    <p><strong>Sender Wallet:</strong> {selectedDonation.sender?.walletAddress || "Not set"}</p>
                    ...
                    {showUserDropdown && (
                      <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                        ...
                        <div className="p-2">
                          ...
                          <LogoutButton
                            logoutUrl="http://localhost:5500/api/v1/users/logout"
                            redirectTo="/"
