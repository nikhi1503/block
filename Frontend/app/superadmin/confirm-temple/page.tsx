...
export default function ConfirmPage() {
  ...
  const fetchPendingConfirmations = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:5500/api/v1/superAdmin/get-pending-confirmations",
        {
          method: "GET",
          headers: {
  ...
  const handleConfirmAndRegister = async (
    ...
    try {
      ...
      if (success) {
        ...
        const response = await fetch(
          "http://localhost:5500/api/v1/superAdmin/confirm-temple-admin-registration",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleReject = async (templeAdminId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5500/api/v1/superAdmin/reject-temple-admin-registration",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleConfirm = (connectionId) => {
    console.log(`Confirming network connection with ID: ${connectionId}`);
    // Find the connection and confirm it
    const connection = pendingNetworkConnections.find(conn => conn.id === connectionId);
    if (connection) {
      toast.success(`Confirmed: ${connection.templeName}`);
      // Here you can add logic to update the status or call an API
    } else {
