    ...
    useEffect(() => {
        const interval = setInterval(async () => {
            try {
                const endpoint =
                    role === "superAdmin"
                        ? "http://localhost:5500/api/v1/superAdmin/refresh-Access-Token"
                        : role === "templeAdmin"
                        ? "http://localhost:5500/api/v1/templeAdmin/refresh-token"
                        : "http://localhost:5500/api/v1/users/refresh-Token";
