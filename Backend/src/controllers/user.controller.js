...
const verifyEmailWithOtp = asyncHandler(async (req, res) => {
    ...
    // If already verified, check OTP anyway and allow to proceed
    if (user.status === 'active') {
        // For already verified users, still validate the OTP if it exists
        if (user.resetOtp && user.resetOtp !== otp) {
            throw new ApiError(400, "Invalid OTP");
        }
        // User is already verified, generate tokens and proceed to next step
        ...
        return res.status(200)
            ...
            .json(
                new ApiResponse(
                    ...
                    "Email already verified. Please proceed to connect wallet."
                )
            );
    }
